...
translation_key$unit <- sub("\\s.*$", "", translation_key$ID)
translation_key$unit <- gsub("[^0-9]", "", translation_key$unit)
...
preferences$unit <- gsub("[^0-9]", "", preferences$unit)
...
translations$unit <- sub(".*/", "", translations$file)
translations$unit <- sub("_[^_]*$", "", translations$unit)
translations$unit <- gsub("[^0-9]", "", translations$unit)
translations$linguist <- sapply(translations$file, function(x) strsplit(x, "/")[[1]][1])
translations$translation_id <- sub(".*/", "", translations$file)
translations$translation_id <- sub("^.*_", "", translations$translation_id)
...
translations <- left_join(translations, preferences |> dplyr::select(-'why'), by=c("linguist", "unit"))
translations <- translations |>
	mutate(preference = ifelse(preference_version == translation_id, 1, -1) * preference_strength) |>
	dplyr::select(-c(preference_version, preference_strength))
...
translations <- translations |> dplyr::select(-c('HT', 'GPT', 'HT EWC', 'GPT EWC'))
```
...
p <- translations |>
	ggplot(aes(x = as.numeric(factor(unit)), y = EC_nonneutral)) +
		geom_rect(data = background_data, xmin = background_data$xmin, xmax = background_data$xmax, ymin = -Inf, ymax = Inf, fill = background_data$fill, inherit.aes = FALSE, alpha = 0.5) +
		geom_point(aes(fill = label, shape = linguist), position = position_dodge(width = 0.8), size = 3, color = "black", alpha = 0.8) +
		scale_y_continuous(breaks = scales::pretty_breaks(n = 5)) +
		labs(x = "Translation", y = "Number of Non-neutral Errors", shape = NULL, fill = NULL, title = "Distribution of Non-neutral Errors by Translation") +
		scale_fill_manual(values = c(col_gpt, col_human, col_neutral)) +
...
p <- translations |>
	ggplot(aes(x = as.numeric(factor(unit)), y = CQS)) +
		geom_rect(data = background_data, xmin = background_data$xmin, xmax = background_data$xmax, ymin = -Inf, ymax = Inf, fill = background_data$fill, inherit.aes = FALSE, alpha = 0.5) +
		geom_point(aes(fill = label, shape = linguist), position = position_dodge(width = 0.8), size = 3, color = "black", alpha = 0.8) +
		labs(x = "Translation", y = "MQM Score", shape = NULL, fill = NULL, title = "Distribution of MQM Scores by Translation") +
		scale_fill_manual(values = c(col_gpt, col_human, col_neutral)) +
		scale_shape_manual(values = c(21:25)) +
		scale_x_continuous(breaks = unique(as.numeric(factor(translations$unit)))) +
...
p <- translations |>
	filter(label == "GPT") |> 
	ggplot(aes(x = as.numeric(factor(unit)) + as.numeric(factor(linguist)) * 0.3 - 0.6, y = preference, shape = linguist)) +
		scale_x_continuous(breaks = unique(as.numeric(factor(translations$unit))),
		...
		labs(x = "Translation Unit", y = NULL, shape = NULL) +
		theme_bw()
...
p <- p + geom_point(size = 2, fill = "black")

p
ggsave("preference_distribution.png", plot = p, width = 8, height = 3, dpi = 1200, bg = "white")
```
...
radar_means <- radar_data |> dplyr::select(-contains("sd")) |> dplyr::select(-"label")

# Proportion of errors (out of total number of errors)
radar_table <- as.data.frame(
  rbind(
    rep(ceiling(max((radar_means * 100) / rowSums(radar_means)) / 20) * 20, ncol(radar_means)),
    rep(0, ncol(radar_means)),
    (radar_means * 100) / rowSums(radar_means)
  )
)
...
text(
  x = 0.0375,
  y = seq(0.25,1,length.out=length(labels)) + 0.05,
  labels = labels,
  adj = c(0, 0.5),
  col = "white",
  cex = 0.625,
...
equivtesting <- translations |> 
	dplyr::select(unit, linguist, label, RQS, CQS) |> 
...
p <- ggplot(data = equivtesting_ci, aes(x = mean_diffCQS, y = n)) +
  xlim(min(equivregion[1]-1,equivtesting_ci$ciRQS[[1]]$conf.int[1]),max(equivregion[2]+1,equivtesting_ci$ciRQS[[1]]$conf.int[2])) +
	geom_rect(aes(xmin = equivregion[1], xmax = equivregion[2], ymin = -Inf, ymax = Inf, fill = "Equivalence area"), alpha = 0.1) +
	geom_vline(xintercept=c(equivregion[1], equivregion[2]), linetype="dashed", color="red") +
	geom_vline(xintercept=0) +
	geom_point(size=6, shape=15) +
	geom_errorbarh(aes(xmin = ci$conf.int[1], xmax = ci$conf.int[2]), height = 0.2, linewidth = 1) +
	labs(y = "", x = "Difference: GPT-4o score – Human score", title = "") +
...
translations_paired <- translations |>
	dplyr::select(c('unit', 'linguist', 'CQS', 'label')) |>
	pivot_wider(names_from = label, values_from = CQS)
wilcox.test(translations_paired$Human, translations_paired$GPT, paired=TRUE, alternative = "two.sided", conf.level = 0.95)

ec_columns <- names(translations)[startsWith(names(translations), "EC_")]
ttest_results <- lapply(ec_columns, function(col_name) {
  translations_paired <- translations |>
    dplyr::select(c('unit', 'linguist', all_of(col_name), 'label')) |> 
    group_by(unit, label) |> 
  ...
  print(wilcox.test(translations_paired$Human, translations_paired$GPT, paired = TRUE, alternative = "two.sided", conf.level = 0.95))
})
...
# load dataframe
df <- translations |> dplyr::select(unit, linguist, label, CQS)
...
# long to wide
df_wide <- reshape(df, idvar = "ID", timevar = "linguist", direction = "wide", v.names = "CQS")

df_human_wide <- reshape(df_human, idvar = "ID", timevar = "linguist", direction = "wide", v.names = "CQS")

df_GPT_wide <- reshape(df_GPT, idvar = "ID", timevar = "linguist", direction = "wide", v.names = "CQS")
...
```{r Intraclass Correlation Coefficient - Preference}
library(irr)

# load dataframe
df <- read_excel('preferences.xlsx') |> as.data.frame()

# Create a new column to hold the numerical scores
df <- df |> mutate(preference_score = case_when(
    `Strongly Prefer Translation 1` == "x" ~ 1,
    `Prefer Translation 1` == "x" ~ 1,
    `No preference` == "x" ~ 2,
    `Prefer Translation 2` == "x" ~ 3,
    `Strongly Prefer Translation 2`== "x" ~ 3,
    .default = NA
))

# Reshape the data
df_fleiss <- reshape(df[, c("unit", "linguist", "preference_score")],
                     idvar = "unit", timevar = "linguist", direction = "wide")
df_fleiss <- df_fleiss[, -1]
df_kripp <- df_fleiss

# Calculate Fleiss' Kappa
kappa_results <- kappam.fleiss(df_fleiss)
print(kappa_results)

# Calculate Kripp's Alpha
kripp_alpha <- kripp.alpha(as.matrix(df_kripp))
print(kripp_alpha)
```
