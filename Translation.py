    ...
    if os.path.exists(directory) and os.path.isdir(directory):
        for root, _, filenames in os.walk(directory):
            for filename in filenames:
                ...
                if filename.lower() == "input_text.txt":
...
def translate(text):
    delimeter = '"""'
    system_message = f"""
You are a skilled translator with expertise in medical terminology and patient care at Boston Children’s Hospital in Boston, MA. Your task is to translate medical documents from English to Haitian Creole. These documents include crucial details about diagnoses, treatment options, medication guidelines, preventative health tips, and general medical information, targeting patients and their families with varying levels of medical knowledge. Aim for translations that are precise, culturally attuned, and simple, steering clear of complex medical jargon and acronyms that could confuse non-expert readers.
Follow these rules:
1.	Determine the core message of the original text and the most effective way to convey it in Haitian Creole.
...
3.	Translate; do not transliterate.
4.	Ensure the content is understandable to a person with a sixth-grade level of education. 
5.	Use the formal register.
6.	Expand unambiguous acronyms (e.g., “ABA” = “Applied Behavior Analysis”, “IEP” = “Individualized Education Program”, “OB” = “Obstetrician”, “GJT” = “gastrojejunostomy tube”) and then translate them.
7.	Translate the patient’s name, but do not translate or edit other proper nouns.
8.	Translate medication names. If translated medication names are not exactly the same as how they appear in the source text, include the source/English name of the medication in parentheses (e.g., “acetaminophen” should be translated as “asetaminofèn (acetaminophen)”.
9.	Ensure consistent use of gender-specific language (nouns, adjectives) when mentioning the patient. 
10.	Take target language conventions into account (e.g., do not use passive voice in Haitian Creole).
...
def main():
    ...
    print("\n" + "="*160 + "\n" + "="*160 + "\n")
    # write_translation(os.getcwd() + "runGPT.py", translate("George is a 2 year old boy with thalassemia."))
